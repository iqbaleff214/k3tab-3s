            ...
            return DataTables::of($data->get())
                ...
                ->addColumn('action', function ($row) {
                    ...
                    if ($row->request_status === 0) {
                        $action = '<form class="d-inline" method="POST" action="' . route('toolman.request.consumable.update', $row) . '">
                                    ...
                                    <form class="d-inline" method="POST" action="' . route('toolman.request.consumable.update', $row) . '">
                                        <input type="hidden" name="_method" value="PUT">
                                        <input type="hidden" name="_token" value="' . csrf_token() . '" />
                ...
                ->rawColumns(['request_status', 'action', 'created_at', 'consumable', 'mechanic'])
                ->make(true);
        }

        return view('pages.toolman.request.consumable', [
    ...
    public function update(Request $request, RequestConsumable $consumable)
    {
        try {
            ...
            $consumable->consumable()->update([
                'quantity' => $quantity - $accepted_quantity,
            ]);
